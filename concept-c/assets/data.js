...
/* ---------------------------------------------------------------------------
 * GESI-STYLE THEME SPINE, used only by Concept C
 * ---------------------------------------------------------------------------
 * gender.cgiar.org organises everything under ten named research themes, each
 * with a short list of sub-topics beneath it, and nothing else on the page
 * competes with that list. Concept C mirrors that structure.
 *
 * IMPORTANT, and stated on the page itself: these ten themes are an EDITORIAL
 * GROUPING built from the tags already present in the 44 catalogued items above.
 * They are not a published CGIAR taxonomy, and no climate-terms taxonomy exists
 * yet. They are the direct equivalent of the three hero categories on the v0.2
 * prototype, which carry the same caveat in that code. Relabel freely.
 *
 * Item counts are matched at runtime from the catalogue, never typed in, so a
 * theme showing zero is telling the truth about the catalogue.
 * --------------------------------------------------------------------------- */
window.HUB.themes = [
  ...
  { name: 'Seasonal forecasts and advisories',
    ...
    match: ['forecast', 'enso', 'niño', 'nino', 'advisor', 'anticipatory', 'seasonal'] },
  { name: 'Climate finance and policy',
    sub: ['GCF climate rationale', 'readiness funding', 'investment targeting', 'negotiations'],
    match: ['gcf', 'finance', 'fund', 'policy', 'readiness', 'adaptation fund'] },
  ...
  { name: 'Landscapes and biodiversity',
    ...
    match: ['landscape', 'geospatial', 'digital twin', 'land management', 'slm', 'biodivers'] },
  { name: 'Data, methods and standards',
    sub: ['metadata and catalogues', 'runnable tutorials', 'agent and API access', 'analytical pipelines'],
    match: ['metadata', 'catalogue', 'tutorial', 'agent', 'api', 'method', 'pipeline', 'standard', 'data portal'] }
];

/* Count catalogue items against a theme. Substring matching over the same fields
   the site search uses, so a reviewer can reproduce any number by searching. */
window.HUB.themeItems = function (theme) {
  return window.HUB.all.filter(function (i) {
    var hay = [i.title, i.blurb, i.kind, i.source, (i.tags || []).join(' ')].join(' ').toLowerCase();
...
/* The four routes gender.cgiar.org offers as "your preferred vantage point",
   translated to what this catalogue actually holds. */
window.HUB.vantage = [
  { label: 'Stories of change',
    desc: 'How climate research reaches decisions and, eventually, fields.',
    gesi: 'How gender research unfolds and impacts people\'s lives',
    types: [], note: 'Nothing to show. No story or impact narrative has been written for this Hub yet.' },
  { label: 'Distilled evidence',
    desc: 'Briefs and syntheses, suited to fast reading.',
    gesi: 'Distilled evidence, especially suited for fast consumption and use',
    types: ['publication'] },
  { label: 'Tools, methods and training',
    desc: 'How to do the analysis, and what to do it with.',
    gesi: 'Tools, methods and training materials',
    types: ['method', 'innovation'] },
  { label: 'Scientific evidence and data',
    desc: 'The datasets and the peer-reviewed record behind them.',
    gesi: 'Scientific evidence and data',
    types: ['dataset', 'publication'] }
];

/* Image credits. Every photograph on Concept C is Creative Commons and carries
   its attribution in the markup, per the licence and per the round 2 principle
   of keeping attribution. Source records are in the RCA Dashboard Image Library. */
window.HUB.credits = {
